...
public class FraudDetectionService {
    ...
    public void checkTransaction(Map<String, Object> payload) {
        ...
        if(result.isFraud()){
            ...
            kafkaTemplate.send(VERIFICATION_REQUIRED_TOPIC, transactionId);
        }
        else {
    ...
    private boolean isAmountSuspicious(String accountNumber, BigDecimal amount) {
        ...
        BigDecimal newArg = avgAmount.add(amount).divide(BigDecimal.valueOf(2),2, RoundingMode.HALF_UP);
        redisTemplate.opsForValue().set(avgKey, newArg.toString());
