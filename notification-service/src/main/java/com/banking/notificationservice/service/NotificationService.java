...
public class NotificationService {
    ...
    public void consumeOtpGenerated(@Payload Map<String,Object> payload) {
        try {
            ...
            sendAlert(accountNumber,"TRANSACTION VERIFICATION REQUIRED",String.format("Suspicious detected on your account" + "reason: %s" + "A transaction of %s is pending verification." + "Your OTP is: %s. Valid for 5 minutes" + "If this wasn't you - ignore this message"));
    ...
    public void consumeTransactionRefunded(@Payload Map<String,Object> payload) {
        try {
            ...
            sendAlert(senderAccount,"REFUND PROCESS", String.format("Your transaction of %s was cancelled"
                                                                             + "Reason: %s " +
    ...
    public void consumePaymentCompleted(@Payload Map<String,Object> payload) {
        try {
            ...
            sendAlert(accountNumber,"PAYMENT SUCCESSFULLY",String.format("Payment of %s completed" + "Razorpay ID: %s",amount,payload.get("razorpayPaymentId")));
        } catch (Exception e) {
    ...
    public void consumePaymentFailed(@Payload Map<String,Object> payload) {
        try {
            ...
            sendAlert(accountNumber,"PAYMENT FAILED",String.format("Payment of %s could not be processed" + "Please try again or contact support",amount));
        } catch (Exception e) {
